...
class IterationCallback(LearnerTensorboardWriter):
    ...
    def __init__(
        ...
        learn: Learner,
        name: str = "model",
        checpoint_keep_num=5,
        show_iters: int = 50,
        eval_iters: int = 1000,
        save_iters: int = 20000,
        start_iters: int = 0,
        stats_iters=20000,
    ):
        ...
        super().__init__(
            ...
            base_dir=".",
            name=learn.path,
            loss_iters=show_iters,
            stats_iters=stats_iters,
            hist_iters=stats_iters,
        )
    ...
    def _write_metrics(
        self, iteration: int, names: List[str], last_metrics: MetricsList
    ) -> None:
    ...
    def on_batch_end(self, iteration, epoch, last_loss, smooth_loss, train, **kwargs):
        ...
        if iteration % self.loss_iters == 0:
            ...
            self.tbwriter.add_scalar(
                tag=self.metrics_root + "lr",
                scalar_value=self.opt.lr,
                global_step=iteration,
            )

        if iteration % self.show_iters == 0:
            log_str = (
                f"epoch {epoch} iter {iteration}: loss = {last_loss:6.4f},  "
        ...
        if iteration % self.eval_iters == 0:
            ...
            # 1. Record time
            log_str = (
                f"average data time = {self.timer.average_data_time():.4f}s, "
            ...
            log_str = (
                f"epoch {epoch} iter {iteration}: eval loss = {last_metrics[0]:6.4f},  "
            ...
            if current is not None and current > self.best:
                logging.info(
                    f"Better model found at epoch {epoch}, "
        ...
        if iteration % self.save_iters == 0 and self.host:
            ...
            if len(checkpoints["all_checkpoints"]) > self.checpoint_keep_num:
                ...
                removed_checkpoint = (
                    self.learn.path / self.learn.model_dir / f"{removed_checkpoint}.pth"
                )
...
class TextAccuracy(Callback):
    ...
    def on_epoch_begin(self, **kwargs):
        self.total_num_char = 0.0
        self.total_num_word = 0.0
        self.correct_num_char = 0.0
        self.correct_num_word = 0.0
        self.total_ed = 0.0
        self.total_ned = 0.0
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        ...
        # ).all(), f"{pt_lengths} != {pt_lengths_} for {pt_text}"
        last_output = self._update_output(
            last_output, {"pt_text": pt_text, "pt_scores": pt_scores}
        )

        pt_text = [self.charset.trim(t) for t in pt_text]
        label = last_target[0]
        if label.dim() == 3:
            label = label.argmax(dim=-1)  # one-hot label
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        mets = [
            self.correct_num_char / self.total_num_char,
...
class TopKTextAccuracy(TextAccuracy):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        ...
        for logit, pt_length, label, length in zip(
            logits, pt_lengths, gt_labels, gt_lengths
        ):
            word_flag = True
...
class DumpPrediction(LearnerCallback):
    ...
    def __init__(
        self, learn, dataset, charset_path, model_eval, image_only=False, debug=False
    ):
        ...
        self.dataset, self.root = (
            dataset,
            Path(self.learn.path) / f"{dataset}-{self.model_eval}",
        )
    ...
    def on_batch_end(self, last_input, last_output, last_target, **kwargs):
        ...
        for gt, pt, image, attn, logit in zip(
            gt_text, pt_text, images, attn_scores, logits
        ):
            ...
            if gt != pt:
                if self.debug:
                    ...
                    logging.info(
                        f"{self.c} gt {gt}, pt {pt}, logit {logit.shape}, scores {scores.topk(5, dim=-1)}"
                    )
            ...
            if not self.image_only:
                ...
                attn_sum = np.array([np.array(a) for a in attn_pil[: len(pt)]]).sum(
                    axis=0
                )
                blended_sum = self.tensor(blend_mask(image_np, attn_sum))
                blended = [
                    self.tensor(blend_mask(image_np, np.array(a))) for a in attn_pil
                ]
                ...
                vutils.save_image(
                    save_image,
                    self.attn_root / f"{self.c}_{gt}_{pt}.jpg",
                    nrow=2,
                    normalize=True,
                    scale_each=True,
                )
