...
def _get_training_phases(config, n):
    ...
    phases = [
        TrainingPhase(n * periods[i]).schedule_hp("lr", lr * sigma[i])
...
def _get_learner(config, data, model, local_rank=None):
    ...
    if config.global_conclr:
        Losses = TotalLosses(
            one_hot=config.dataset_one_hot_y,
            tau=config.training_tau,
            alpha=config.training_alpha,
        )
    else:
        Losses = MultiLosses(one_hot=config.dataset_one_hot_y)
    learner = Learner(
        data,
        model,
        silent=True,
    ...
    if config.global_phase == "train":
        ...
        phases = _get_training_phases(
            config, len(learner.data.train_dl) // num_replicas
        )
...
def main():
    ...
    if args.name is not None:
        config.global_name = args.name
    if args.phase is not None:
        config.global_phase = args.phase
    if args.test_root is not None:
        config.dataset_test_roots = [args.test_root]
    if args.checkpoint is not None:
        config.model_checkpoint = args.checkpoint
    if args.debug is not None:
        config.global_debug = args.debug
    if args.image_only is not None:
        config.global_image_only = args.image_only
    if args.model_eval is not None:
        config.model_eval = args.model_eval
    if args.model_strict is not None:
        config.model_strict = args.model_strict
    ...
    if config.global_phase == "train":
        ...
        log_str = (
            f"eval loss = {last_metrics[0]:6.3f},  "
