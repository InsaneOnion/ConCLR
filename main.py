...
from callbacks import DumpPrediction, IterationCallback, TextAccuracy, TopKTextAccuracy, ConAugPretransform
...
def _get_learner(config, data, model, local_rank=None):
    ...
    if config.global_conclr:
        Losses = TotalLosses(one_hot=config.dataset_one_hot_y, tau=config.training_tau, alpha=config.training_alpha)
    else:
    ...
    if config.global_phase == 'train':
        ...
        if config.global_conclr:
            learner.callback_fns += [partial(ConAugPretransform, aug_type=config.dataset_conaug_type, max_length=config.dataset_max_length)]
    else:
        learner.callbacks += [
